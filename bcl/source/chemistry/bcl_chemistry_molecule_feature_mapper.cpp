...
//            m_LocalMap[ atom_index] = score_diff;
            ...
            ////////// SINGLE ATOM REPLACE ////////// 187-425ish
          for( size_t index( m_StartIndex), max_index( m_PerturbAtoms.GetSize()); index < max_index; index += m_Stride)
          {
            size_t atom_index( m_PerturbAtoms( index));
            if
            (
                ...
                atom_vector( atom_index).GetAtomType()->GetElementType() == GetElementTypes().e_Carbon                       // skip Carbon-to-Carbon replacement
            ...
            // Bayesian smoothening with Dirichlet prior to ensure no 0's when calculating KL Divergence
            linal::Vector< float> parent_score_kl_smoothed( parent_score_kl.GetSize());
            linal::Vector< float> score_kl_smoothed( score_kl.GetSize());
            float parent_score_kl_sum( parent_score_kl.Sum());
            float score_kl_sum( score_kl.Sum());

            for( size_t itr( 0); itr < parent_score_kl.GetSize(); ++itr)
            {
              parent_score_kl_smoothed( itr) = (parent_score_kl( itr) + 0.1) / (parent_score_kl_sum + 0.1*parent_score_kl.GetSize() );
              score_kl_smoothed( itr) = (score_kl( itr) + 0.1) / (score_kl_sum + 0.1*score_kl.GetSize() );
            }
            BCL_MessageDbg( "Parent smoothed distribution sum: " + util::Format()( parent_score_kl_smoothed.Sum()));
            BCL_MessageDbg( "Perturbed smoothed distribution sum: " + util::Format()( score_kl_smoothed.Sum()));

            for( size_t itr( 0); itr < parent_score_kl.GetSize(); ++itr)
            {
              kl_div_comps( itr) = parent_score_kl_smoothed( itr) * log( parent_score_kl_smoothed( itr) / score_kl_smoothed( itr));
            }

            // 20240213 TRIAL: output = m*(KL) vs floor( m*(KL)) vs e^( floor( m*(|KL|))
            // lin vs flin vs exp: lin = linear, i.e. not an e^X expression. exp = exponential, e^X expression.
            // hardFilter vs softFilter: how significant an effect must be to affect the output heatmap.

            // First, the unadjusted linear output:
            ...
            // Second, the Floor-filtered linear output:
            float score_diff_kl_lin_hardFilter( floorf( score_diff_kl));
            float score_diff_kl_lin_softFilter( floorf( 5 * score_diff_kl));

            // Third, the unfiltered exponential output:
            float score_diff_kl_exp_A( expf( sqrtf( score_diff_kl * score_diff_kl)) - 1);
            float score_diff_kl_exp_D( expf( 5 * sqrtf( score_diff_kl *score_diff_kl)) - 1);

            // Fourth, the Floor-filtered exponential output:
            float score_diff_kl_exp_hardFilter( expf( sqrtf( floorf(score_diff_kl) * floorf(score_diff_kl)))  - 1);
            float score_diff_kl_exp_softFilter( expf( sqrtf( floorf(5 * score_diff_kl) * floorf(5 * score_diff_kl)))  - 1);


            BCL_MessageDbg( "***********M = 1: " + util::Format()( score_diff_kl_exp_A));
            BCL_MessageDbg( "***********M = 5: " + util::Format()( score_diff_kl_exp_D));
            BCL_MessageDbg( "***********M = 1 +FLOOR: " + util::Format()( score_diff_kl_exp_hardFilter));
            BCL_MessageDbg( "***********M = 5 +FLOOR: " + util::Format()( score_diff_kl_exp_softFilter));

            // END 20240213 Trial.

            m_LocalMap[ atom_index] = floorf( 10 * score_diff_kl_exp_D) / 10;
          }
...
//          BCL_MessageDbg( "Parent molecule score distribution: " + util::Format()( parent_score_kl));
//
//          int pow_set_size( pow( 2, m_PerturbAtoms.GetSize()));
...
//
////            float score_exponential( expf( floorf( 2 * score_diff_kl)) - 1);
////            KL_powset.PushBack( score_exponential);
////            BCL_MessageDbg( "Exponential KL score: " + util::Format()( score_exponential));
//
//
...
//
//          // per-atom SCORING: turn molecule-level KL Divergence into Atom_i score by considering KLs of all molecules in which Atom_i was replaced
//          // KL_max and KL_avg as defined above.
//          // KL_max seems to work better, especially for large molecules.
//
//
//          // TRIAL-normsum: An atom's normsum is the sum of that atom's KL contributions normalized to the maximum sum for that molecule. E.g., for sum = (1 2 1 5), normsum = (0.2, 0.4, 0.2, 1.0)
//          storage::Vector< float> temp_vector_sums;
//          storage::Vector< float> temp_vector_Exp_1_nf_sums;
//          storage::Vector< float> temp_vector_Exp_5_nf_sums;
//          storage::Vector< float> temp_vector_Exp_5_f_sums;
//
//          for( int itr_i = 0, itr_i_max = m_PerturbAtoms.GetSize(); itr_i < itr_i_max; ++itr_i)
//          {
//            // 20240213 TRIAL: score filtering. Linear vs exponential (Lin vs Exp), floor vs no floor (f vs nf)
//            // "Linear" output = m * KL.
//            // "Exponential" output = e^( m* |KL|) - 1.
//            // "Floor" means floor( m*KL) or floor( m*|KL|) based on Lin vs Exp context.
//
//            // temp_vector is the original vector, Lin_nf, m = 1. Others named formulaically.
//            storage::Vector< float> temp_vector;
//            storage::Vector< float> temp_vector_Exp_1_nf;
//            storage::Vector< float> temp_vector_Exp_5_nf;
//            storage::Vector< float> temp_vector_Exp_5_f;
//
//
//            for (int itr_j = 0; itr_j < pow_set_size; ++itr_j)
//            {
...
//                 temp_vector_Exp_1_nf.PushBack( expf( sqrtf( KL_powset( itr_j) * KL_powset( itr_j))) - 1);
//                 temp_vector_Exp_5_nf.PushBack( expf( 5 * sqrtf( KL_powset( itr_j) * KL_powset( itr_j))) - 1);
//                 temp_vector_Exp_5_f.PushBack( expf( sqrtf( floorf( 5 * KL_powset( itr_j)) * floorf( 5 * KL_powset( itr_j)))) - 1);
//              }
//            }
//            temp_vector.Sort( std::greater< float>() );
//            temp_vector_Exp_1_nf.Sort( std::greater< float>() );
//            temp_vector_Exp_5_nf.Sort( std::greater< float>() );
//            temp_vector_Exp_5_f.Sort( std::greater< float>() );
//
//            BCL_MessageDbg("**Perturbed Atom " + util::Format()( itr_i) + " KL_set: " + util::Format()( temp_vector));
//            BCL_MessageDbg("**Perturbed Atom " + util::Format()( itr_i) + " KL_set EXP1NF: " + util::Format()( temp_vector_Exp_1_nf));
//            BCL_MessageDbg("**Perturbed Atom " + util::Format()( itr_i) + " KL_set EXP5NF: " + util::Format()( temp_vector_Exp_5_nf));
//            BCL_MessageDbg("**Perturbed Atom " + util::Format()( itr_i) + " KL_set EXP5F: " + util::Format()( temp_vector_Exp_5_f));
//
//            float temp_vector_sum( 0);
//            float temp_vector_Exp_1_nf_sum( 0);
//            float temp_vector_Exp_5_nf_sum( 0);
//            float temp_vector_Exp_5_f_sum( 0);
//
////            float temp_vector_max( temp_vector.FirstElement());
////            float temp_vector_Exp_1_nf_max( temp_vector_Exp_1_nf.FirstElement());
////            float temp_vector_Exp_5_nf_max( temp_vector_Exp_5_nf.FirstElement());
////            float temp_vector_Exp_5_f_max( temp_vector_Exp_5_f.FirstElement());
//
//
//            for( size_t itr( 0), itr_max( temp_vector.GetSize()); itr < itr_max; ++itr)
//            {
//              temp_vector_sum += temp_vector( itr);
//              temp_vector_Exp_1_nf_sum += temp_vector_Exp_1_nf( itr);
//              temp_vector_Exp_5_nf_sum += temp_vector_Exp_5_nf( itr);
//              temp_vector_Exp_5_f_sum += temp_vector_Exp_5_f( itr);
//            }
//
//            float temp_vector_avg( temp_vector_sum / temp_vector.GetSize());
//            float temp_vector_Exp_1_nf_avg( temp_vector_Exp_1_nf_sum / temp_vector_Exp_1_nf.GetSize());
//            float temp_vector_Exp_5_nf_avg( temp_vector_Exp_5_nf_sum / temp_vector_Exp_5_nf.GetSize());
//            float temp_vector_Exp_5_f_avg( temp_vector_Exp_5_f_sum / temp_vector_Exp_5_f.GetSize());
//
//            // TRIAL-normsum
//            temp_vector_sums.PushBack( temp_vector_sum);
//            temp_vector_Exp_1_nf_sums.PushBack( temp_vector_Exp_1_nf_sum);
//            temp_vector_Exp_5_nf_sums.PushBack( temp_vector_Exp_5_nf_sum);
//            temp_vector_Exp_5_f_sums.PushBack( temp_vector_Exp_5_f_sum);
//
//            // Debug readouts
//            BCL_MessageDbg("***Perturbed Atom " + util::Format()( itr_i) + " KL_sum: " + util::Format()( temp_vector_sum));
//            BCL_MessageDbg("***Perturbed Atom " + util::Format()( itr_i) + " KL_sum_EXP1NF: " + util::Format()( temp_vector_Exp_1_nf_sum));
//            BCL_MessageDbg("***Perturbed Atom " + util::Format()( itr_i) + " KL_sum_EXP5NF: " + util::Format()( temp_vector_Exp_5_nf_sum));
//            BCL_MessageDbg("***Perturbed Atom " + util::Format()( itr_i) + " KL_sum_EXP5F: " + util::Format()( temp_vector_Exp_5_f_sum));
//
//            BCL_MessageDbg("***Perturbed Atom " + util::Format()( itr_i) + " KL_avg: " + util::Format()( temp_vector_avg));
//            BCL_MessageDbg("***Perturbed Atom " + util::Format()( itr_i) + " KL_avg_EXP1NF: " + util::Format()( temp_vector_Exp_1_nf_avg));
//            BCL_MessageDbg("***Perturbed Atom " + util::Format()( itr_i) + " KL_avg_EXP5NF: " + util::Format()( temp_vector_Exp_5_nf_avg));
//            BCL_MessageDbg("***Perturbed Atom " + util::Format()( itr_i) + " KL_avg_EXP5F: " + util::Format()( temp_vector_Exp_5_f_avg));
//
//            size_t atom_index( m_PerturbAtoms( itr_i));
//            m_LocalMap[ atom_index] = temp_vector_avg; // temp_max or temp_avg
//
//          }
//
//          // TRIAL-normsum
//          temp_vector_sums.Sort(std::greater< float>() );
//          temp_vector_Exp_1_nf_sums.Sort(std::greater< float>() );
//          temp_vector_Exp_5_nf_sums.Sort(std::greater< float>() );
//          temp_vector_Exp_5_f_sums.Sort(std::greater< float>() );
//
//          float temp_vector_sums_max( temp_vector_sums.FirstElement());
//          float temp_vector_Exp_1_nf_sums_max( temp_vector_Exp_1_nf_sums.FirstElement());
//          float temp_vector_Exp_5_nf_sums_max( temp_vector_Exp_5_nf_sums.FirstElement());
//          float temp_vector_Exp_5_f_sums_max( temp_vector_Exp_5_f_sums.FirstElement());
//
//          for( int itr_i = 0, itr_i_max = m_PerturbAtoms.GetSize(); itr_i < itr_i_max; ++itr_i)
//          {
//            temp_vector_sums( itr_i) = temp_vector_sums( itr_i) / temp_vector_sums_max;
//            temp_vector_Exp_1_nf_sums( itr_i) = temp_vector_Exp_1_nf_sums( itr_i) / temp_vector_Exp_1_nf_sums_max;
//            temp_vector_Exp_5_nf_sums( itr_i) = temp_vector_Exp_5_nf_sums( itr_i) / temp_vector_Exp_5_nf_sums_max;
//            temp_vector_Exp_5_f_sums( itr_i) = temp_vector_Exp_5_f_sums( itr_i) / temp_vector_Exp_5_f_sums_max;
//
////            size_t atom_index( m_PerturbAtoms( itr_i));
////            m_LocalMap[ atom_index] = m_LocalMap[ atom_index] / temp_vector_sums_max;
//          }
//
//          BCL_MessageDbg("****Normsums vanilla " + util::Format()( temp_vector_sums));
//          BCL_MessageDbg("****Normsums EXP1NF " + util::Format()( temp_vector_Exp_1_nf_sums));
//          BCL_MessageDbg("****Normsums EXP5NF " + util::Format()( temp_vector_Exp_5_nf_sums));
//          BCL_MessageDbg("****Normsums EXP5F " + util::Format()( temp_vector_Exp_5_f_sums));
//
//
        } // RunThread
      }; // Worker struct
